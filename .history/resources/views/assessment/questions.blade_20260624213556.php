    ...
    <div class="max-w-2xl mx-auto space-y-4">
        ...
        <div class="card !py-4">
            <div class="flex items-center justify-between mb-2">
                <span class="text-xs font-semibold text-slate-500">Soal {{ $currentNumber }} dari
                    {{ $totalQuestions }}</span>
                <span
                    class="text-xs font-semibold text-violet-600">{{ round((($currentNumber - 1) / $totalQuestions) * 100) }}%
                    selesai</span>
            </div>
            <div class="w-full h-2 bg-slate-100 rounded-full overflow-hidden">
                <div class="h-full bg-gradient-to-r from-violet-500 to-fuchsia-400 rounded-full transition-all duration-500"
            ...
            <div class="flex items-center gap-1 mt-3 flex-wrap">
                @for ($i = 1; $i <= $totalQuestions; $i++)
                    ...
                    {{ $i < $currentNumber
                        ...
                        : ($i == $currentNumber
                            ? 'bg-violet-900 text-white ring-2 ring-violet-300'
        ...
        <form action="{{ route('assessment.answer') }}" method="POST" id="answerForm">
            ...
            {{-- Flex-wrap (bukan grid kaku): kalau jumlah opsi ganjil, opsi terakhir otomatis center, tidak menyisakan ruang kosong --}}
            <div class="flex flex-wrap justify-center gap-2.5" id="optionList">
                @foreach ($question->options as $idx => $option)
                    <label
                        class="option-label flex items-center gap-3 card !py-3.5 cursor-pointer hover:border-violet-300 border-2 border-transparent transition-all select-none basis-[calc(50%-0.3125rem)] grow-0"
                        ...
                        <div
                            class="option-circle w-9 h-9 rounded-xl border-2 border-slate-200 flex items-center justify-center font-bold text-sm text-slate-400 flex-shrink-0 transition-all">
                            {{ ['A', 'B', 'C', 'D'][$idx] }}
                        </div>
