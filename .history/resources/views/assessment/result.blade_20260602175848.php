...
@section('content')
    <div class="max-w-3xl mx-auto space-y-5">
        ...
        <div class="card relative overflow-hidden">
            <div
                class="absolute top-0 right-0 w-48 h-48 {{ $lc['soft'] }} rounded-full -translate-y-1/2 translate-x-1/4 opacity-60">
            </div>
            <div class="relative z-10 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-5">
                ...
                <div
                    class="flex flex-col items-center {{ $lc['soft'] }} border {{ $lc['border'] }} rounded-2xl px-7 py-4 text-center flex-shrink-0">
        ...
        {{-- ── DRAW NOTE — tampil hanya jika terjadi draw/seri ── --}}
        @if (!empty($result['draw_note']))
            <div class="flex items-start gap-3 bg-amber-50 border border-amber-200 rounded-2xl px-4 py-3">
                <div class="w-7 h-7 bg-amber-100 rounded-lg flex items-center justify-center flex-shrink-0 text-base">⚖️
                </div>
                <div>
                    <p class="font-semibold text-amber-800 text-xs">Catatan: Hasil Seri Antar Level</p>
                    <p class="text-amber-700 text-xs mt-0.5 leading-relaxed">{{ $result['draw_note'] }}</p>
                </div>
            </div>
        @endif


        {{-- ── PROBABILITAS LEVEL — output utama sistem pakar ── --}}
        @if (!empty($result['probabilitas']))
            <div class="card">
                <div class="flex items-center justify-between mb-4">
                    <h3 class="font-bold text-slate-800 text-sm">Distribusi Probabilitas Level</h3>
                    <span class="text-[10px] bg-violet-100 text-violet-600 font-semibold px-2 py-0.5 rounded-full">
                        Rule-Based AI
                    </span>
                </div>
                ...
                @php
                    ...
                    $levelDesc = [
                        'NSI' => 'Perlu Intervensi Khusus',
                        'Basic' => 'Pemahaman Dasar',
                        'Proficient' => 'Cukup Mahir',
                        'Advanced' => 'Sangat Mahir',
                    ];
                @endphp

                <div class="space-y-3">
                    @foreach ($result['probabilitas'] as $level => $pct)
                        @php $pc = $probConfig[$level] ?? $probConfig['NSI']; @endphp
                        <div class="flex items-center gap-3">
                            {{-- Label level --}}
                            <div class="w-24 flex-shrink-0">
                                <span class="text-xs font-bold {{ $pc['text'] }}">{{ $level }}</span>
                                @if ($level === $result['level'])
                                    <span
                                        class="ml-1 text-[9px] bg-violet-100 text-violet-600 font-bold px-1 py-0.5 rounded">✓
                                        Hasil</span>
                                @endif
                            </div>
                            ...
                            <div class="flex-1 h-3 bg-slate-100 rounded-full overflow-hidden">
                                <div class="{{ $pc['bar'] }} h-full rounded-full transition-all duration-700"
                                    style="width: {{ $pct }}%"></div>
                            </div>

                            {{-- Persentase --}}
                            <span class="text-xs font-bold {{ $pc['text'] }} w-12 text-right flex-shrink-0">
                                {{ $pct }}%
                            </span>
                        </div>
                    @endforeach
                ...
                {{-- Penjelasan singkat untuk guru --}}
                <p class="text-[11px] text-slate-400 mt-4 leading-relaxed border-t border-slate-100 pt-3">
                    Persentase menunjukkan kecenderungan kemampuan siswa ke tiap level berdasarkan pola jawaban.
                    Level dengan persentase tertinggi adalah hasil diagnosis utama.
                </p>
            </div>
        @endif
                    ...
                    <div>
                        <div class="flex items-center justify-between mb-1">
                            <div class="flex items-center gap-2">
                                ...
                                @if ($topic['is_weak'])
                                    <span
                                        class="text-[10px] bg-red-100 text-red-600 font-semibold px-1.5 py-0.5 rounded-full">Perlu
                                        Perhatian</span>
                                @endif
                            </div>
                            <span
                                class="text-xs font-bold {{ $badge }} px-2 py-0.5 rounded-lg">{{ $pct }}%</span>
                        </div>
                        <div class="h-2 bg-slate-100 rounded-full overflow-hidden">
                            <div class="{{ $barColor }} h-full rounded-full transition-all duration-700"
                                style="width: {{ $pct }}%"></div>
                        </div>
                        <p class="text-[11px] text-slate-400 mt-0.5">{{ $topic['correct'] }} benar dari
                            {{ $topic['total'] }} soal</p>
                    </div>
                @endforeach
            </div>
